...
//  }
  ...
  /*runahead code begin*/
  (rh_load_resp === true.B)) && !exit_miss_back
  /*runahead code end*/
 ...
 /*runahead code end*/
  ...
  val rf_wdata = 
                /*runahead code begin*/
                 Mux(dmem_resp_valid && dmem_resp_xpu && (runahead_flag === true.B) && (wb_rh_hit === true.B), rhbuffer_data(hit_ptr),
                /*runahead code end*/
...
//checkpoint scoreboard
  ...
  when(runahead_posedge){
      for(i <- 0 until 31) { rf_sbasbools(i.U) := sboard.read(i.U)}
      //for(j <- 0 until 32) { fp_sbasbools(j.U) := fp_sboard.read(j.U)}
  }
  rcu.io.rf_waddr := ll_waddr
  //rcu.io.fp_waddr := io.fpu.sboard_clra   
   when(runahead_flag && ll_wen && rf_sb_wb(ll_waddr)===1.U && invfile(ll_waddr)===0.U)
    {
