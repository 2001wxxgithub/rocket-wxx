...
//  val wb_rh_load_resp = Reg(Bool())
  ...
  val mem_invfile = WireInit(VecInit(Seq.fill(31)(0.U(1.W))))
  val wb_invfile = WireInit(VecInit(Seq.fill(31)(0.U(1.W))))
  ...
  val invstore = RegInit(false.B)    //rh期间是否遇到过无效store，如果遇到则之后的所有指令都为无效
  //val longinst_pc = Reg(UInt())  //保存进入rh后跑的第一个指令
  val rh_source_inv = invstore || (wb_rh_readrf(0) && wb_invfile(wb_rh_raddr(0))===1.U) || 
  (wb_rh_readrf(1) && wb_invfile(wb_rh_raddr(1))===1.U) 
  ...
  val wb_rh_longinst_valid = wb_rh_longinst && !rh_source_inv     //长指令且来源有效，一个指令期间置高1cycle
  ...
  // 去掉了wb_valid &&
  val ex_rh_longinst = ex_ctrl.div || ex_ctrl.mul || (ex_ctrl.fp && ex_ctrl.wfd) || (ex_ctrl.mem && isRead(ex_ctrl.mem_cmd)) 
  val ex_longinst_kill = false.B
  //longinst_wb_flag && ex_rh_longinst && longinst_if_record(ex_rh_step_counter + 1.U)=== 1.U    //需要kill掉ex阶段防止该长指令访存/调用fpu
  //去掉了 && ex_pc_valid
  ...
  /*runahead code begin*/
  // rcu.io.ipc := Mux(!ibuf.io.inst(0).valid && !wb_valid, wb_reg_pc, ibuf.io.pc)//wb_reg_pc
  //  rcu.io.ipc := Mux(runahead_state === s_runahead && (!s2_runahead & s1_runahead), ibuf.io.pc ,rcu.io.ipc)
  rcu.io.wb_valid := db_flag
  //io.fpu.wb_valid := db_flag
  /*runahead code end*/
  ...
  div.io.req.valid := ex_reg_valid && ex_ctrl.div && !db_flag && !ex_longinst_kill   //wxx-runahead防止再次运算长指令调用  && !db_flag
  ...
  val mul = pipelinedMul.option {
    ...
    m.io.req.valid := ex_reg_valid && ex_ctrl.mul && !ex_longinst_kill   //wxx-runahead防止再次运算长指令调用
  ...
  /*runahead code begin*/
  !wb_dmem_req_inv && !exit_miss_back //&& !rh_longinst_wrf && !rh_longinst_wfp //wxx-提前回写记录过的长指令结果时也不会访存
  /*runahead code end*/
...
//  }
  ...
  div.io.kill := killm_common && RegNext(div.io.req.fire) || db_flag   //wxx
...
//    wb_rh_load_resp := mem_rh_load_resp
  ...
  val wb_set_sboard = wb_ctrl.div || wb_dcache_miss || wb_ctrl.rocc //&& !rh_longinst_wrf // wxx 被记录过的长指令提前写回时不用记录sb
 ...
 /*runahead code begin*/
  ...
  val s0_db_flag = runahead_state === s_pass && io.dmem.mshr_state(0) === 0.U && io.dmem.resp.valid && io.dmem.resp.bits.tag(5, 1) === runahead_tag(0)(6, 2) && io.dmem.resp.bits.addr === runahead_addr(0)
 ...
 /*runahead code end*/
  ...
  val dmem_resp_waddr = Mux(wb_rh_hit && !s0_db_flag , wb_waddr, io.dmem.resp.bits.tag(5, 1))    
  /*runahead code end*/
  val dmem_resp_valid = (io.dmem.resp.valid && io.dmem.resp.bits.has_data ) && !exit_miss_back
  /*runahead code begin*/
  /*(wb_rh_load_resp === true.B)*/
  /*runahead code end*/
  ...
  val rf_wdata = 
                /*runahead code begin*/
                 Mux((runahead_flag === true.B) && (wb_rh_hit === true.B) && !s0_db_flag, rhbuffer_data(wb_hit_ptr),   
                 // s0_db_flag周期不能写回rhbuffer_data，会和dmem_resp冲突
                 //Mux(rh_longinst_wrf , rh_step_wdata(rh_step_counter + 1.U),       //wxx-rh长指令提前写回regfile
                /*runahead code end*/
  ...
  when(db_flag) {
    for(j <- 0 until 31) {
      when(j.U =/= runahead_tag(0)(6, 2)){    //bug 需要额外判断是整型tag还是浮点型tag -wxx
  ...
  wb_rh_longinst_stop_record := Mux(runahead_posedge && mem_ctrl.branch || db_flag || s0_db_flag , true.B ,
                                Mux(runahead_posedge, false.B ,             //rh开始和结束时置低 遇到来源无效的分支就置高停止记录长指令的值
                                Mux(!wb_rh_longinst_stop_record, (wb_ctrl.branch||wb_ctrl.jalr) && rh_source_inv, wb_rh_longinst_stop_record)) )
  //每次进入rh都把wb_rh_longinst_stop_record初始化置低，之后遇到inv分支置高
  val longinst_pc = RegEnable(wb_reg_pc, ShiftRegister(runahead_posedge,6))

  rh_step_counter := Mux(runahead_posedge || (!runahead_flag && ibuf.io.pc === longinst_pc && longinst_if_record =/= 0.U) , 0.U,                        //遇到非法分支就停止步长计数   db_flag ||
                     Mux(longinst_wb_flag && wb_rh_longinst && wb_valid && rh_step_counter<20.U , rh_step_counter+1.U,
                     Mux(runahead_flag && !wb_rh_longinst_stop_record && wb_valid && wb_rh_longinst && rh_step_counter<20.U ,rh_step_counter+1.U,rh_step_counter))) //提交长指令后计数+1
  //提交长指令（来源未必有效）就步长+1 ，步长最多为100.U（防止退出rh之后步长不停计数）


  invstore := Mux(runahead_posedge || db_flag , false.B ,
              Mux(runahead_flag && wb_reg_store && rh_source_inv, true.B, invstore))
  ...
  //val test_counter = RegInit(0.U(2.W))
   ...
   when(runahead_flag && !wb_rh_longinst_stop_record){     //从rh开始记录
 //不写入scoreboard的指令
  when(wb_rh_longinst_valid && wb_wen)      //长指令来源有效，wb_wen = wb_valid && wb_ctrl.wxd 写回到rf，要用到rh_rf_waddr_step 
  {
    rh_rf_waddr_step(wb_waddr):= rh_step_counter + 1.U     //初始值为0
    rh_step_wdata(rh_step_counter + 1.U):= Mux(wb_wen && !wb_set_sboard, rf_wdata, rh_step_wdata(rh_step_counter + 1.U)) //直接写回寄存器的指令
    longinst_if_record_sbasbools(rh_step_counter + 1.U) := Mux(wb_wen && !wb_set_sboard, true.B, longinst_if_record(rh_step_counter + 1.U))     }  //该长指令结果已被记录，防止后续重复写入
  // }.elsewhen(wb_rh_longinst_valid && !wb_rh_longinst_stop_record && wb_ctrl.wfd && wb_valid){  //长指令来源有效，写回到fp，要用到rh_fp_waddr_step 
  //   rh_fp_waddr_step(wb_waddr) := rh_step_counter + 1.U                    //对于fp指令，即使不会记录scoreboard，写回的值也不一定立刻返回，所以后续统一处理
  // }
  //fp指令写回的三种情况：立即写回，load,fma
//     when(wb_rh_longinst_valid && io.fpu.fp_wen_wb && !io.fpu.fp_fma && !wb_rh_longinst_stop_record)  //fp_wen_wb:= !wbInfo(0).cp && wen(0) 立刻写回
//   {
//     rh_step_wdata(rh_step_counter + 1.U) := io.fpu.fp_wdata
//     longinst_if_record_sbasbools(rh_step_counter + 1.U) := true.B
//   }.elsewhen(io.fpu.fp_load_wb && rh_fp_waddr_step(io.fpu.fp_waddr)=/=0.U && longinst_if_record(rh_fp_waddr_step(io.fpu.fp_waddr))===0.U ){

//      rh_step_wdata(rh_fp_waddr_step(io.fpu.fp_waddr)) := io.fpu.fp_wdata  
//      longinst_if_record_sbasbools(rh_fp_waddr_step(io.fpu.fp_waddr)) := true.B
//  }.elsewhen(io.fpu.fp_wen_wb && io.fpu.fp_fma && rh_fp_waddr_step(io.fpu.fp_waddr)=/=0.U && longinst_if_record(rh_fp_waddr_step(io.fpu.fp_waddr))===0.U ){
...
//  }
 ...
 //写入scoreboard的指令
  when(ll_wen && (rf_sb_wb(ll_waddr)===0.U || ll_waddr === mshr_l2miss_tag(6, 2))&& rh_rf_waddr_step(ll_waddr)=/=0.U && longinst_if_record(rh_rf_waddr_step(ll_waddr))===0.U ){ //&& longinst_if_record(rh_rf_waddr_step(ll_waddr))===0.U){     // set_sboard后写回的指令   
  ...
  for (i <- 0 until 20) { 
    rh_step_wdata (i) := 0.U 
    longinst_if_record_sbasbools(i) := false.B
  }
  ...
  //io.fpu.l2miss := runahead_posedge
...
//checkpoint scoreboard
  ...
  when(runahead_posedge){
      for(i <- 0 until 31) { rf_sbasbools(i.U) := Mux(ll_wen && i.U===ll_waddr, false.B, sboard.read(i.U))}   //Mux(i===mshr_l2miss_tag(6, 2), 0.U, 
      //for(j <- 0 until 32) { fp_sbasbools(j.U) := fp_sboard.read(j.U)}
  }
  rcu.io.rf_waddr := ll_waddr
  //rcu.io.fp_waddr := io.fpu.sboard_clra   
   when(runahead_flag && ll_wen && rf_sb_wb(ll_waddr)===1.U && invfile(ll_waddr)===0.U && ll_waddr =/= runahead_tag(0)(6, 2))
    {
      rcu.io.rf_sb_valid := true.B     //表明之前未写回的有效的sboard写回了，需要同步
      rf_sbasbools(rf_waddr) := false.B    //完成第一次写回后不需要再更新了
    }.otherwise{rcu.io.rf_sb_valid := false.B}
    // when(runahead_flag && io.fpu.sboard_clr && fp_sb_wb(io.fpu.sboard_clra)===1.U)//when(runahead_flag && wb_fp_wen && fp_sb_wb(wb_waddr)===1.U)
    // {
...
//    Mux(s2_runahead_posedge, wb_reg_pc,
  ...
  !ex_dmem_req_inv && !ex_longinst_kill //wxx-longinst
...
//=======================runahead-wxx=================================//
  ...
  ex_rh_step_counter := Mux(!runahead_flag && ibuf.io.pc === longinst_pc && longinst_if_record =/= 0.U, 0.U ,
                      //Mux(db_flag , 0.U,   //遇到分支预测就停止步长计数   
  ...
  longinst_wb_flag:= Mux(runahead_posedge, false.B ,
                     Mux(longinst_if_record ===0.U , false.B,                     
                     Mux(!runahead_flag , true.B , longinst_wb_flag)))    //rh结束，长指令结果记录完毕，可以开始写回了
  longinst_if_record := longinst_if_record_sbasbools.asUInt
  dontTouch(ex_rh_step_counter)
  dontTouch(longinst_wb_flag)
  //io.fpu.fp_longinst_wbflag := rh_longinst_wfp  //控制写回fpu模块的信号
  when(longinst_wb_flag)
  {
    when(wb_valid && wb_rh_longinst && wb_ctrl.wxd && longinst_if_record(rh_step_counter+1.U)=== 1.U){    //写回整型寄存器：整数乘除，整数load（浮点运算有可能写回整形寄存器吗，应该不会）

        //rf.write(wb_waddr, rh_step_wdata(rh_step_counter + 1.U))
        longinst_if_record_sbasbools(rh_step_counter+1.U) := false.B//longinst_if_record_sbasbools(rh_rf_waddr_step(wb_waddr)) := false.B
    }
    // }.elsewhen(wb_valid && wb_rh_longinst && wb_ctrl.wfd && longinst_if_record(rh_fp_waddr_step(io.fpu.fp_waddr))=== 1.U){    //写回浮点型 : 除法开方，load，其他运算
    //     // io.fpu.fp_longinst_wdata := rh_step_wdata(rh_step_counter + 1.U)
    //     // io.fpu.fp_longinst_waddr := wb_waddr
    //     longinst_if_record_sbasbools(rh_fp_waddr_step(io.fpu.fp_waddr)) := false.B          
    // }   
  }
//=======================runahead-wxx=================================//
