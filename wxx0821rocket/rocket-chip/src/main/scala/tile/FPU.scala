...
class FPUCoreIO(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  // val fp_waddr = Output(UInt(5.W))
  // val fp_wen_wb = Output(Bool()) 
  // val fp_load_wb = Output(Bool()) 
...
class FPU(cfg: FPUParams)(implicit p: Parameters) extends FPUModule()(p) {
  ...
  io.cp_req.ready := !ex_reg_valid
    ...
    //  io.fp_waddr:= Mux(load_wb,load_wb_tag, waddr)
    //  io.fp_wen_wb:= !wbInfo(0).cp && wen(0) 
    //  io.fp_load_wb := load_wb   
  ...
  if (cfg.divSqrt) {
    ...
    val divSqrt_killed =  RegNext(divSqrt_inValid && killm, true.B)      //wxx-runahead  io.wb_valid ||
