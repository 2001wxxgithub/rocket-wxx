...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
 ...
 //============================================runahead-ex-wxx=============================================//
  ...
  val wb_load_req_valid = Reg(Bool())
  //记录读寄存器的地址
  ...
  val rf_ld_wb = RegInit(0.U(32.W))      //
  val fp_ld_wb = RegInit(0.U(32.W))      //记录rh期间load未写回的寄存器号，rh结束后不许它写回
  val rf_ldasbools = VecInit(rf_ld_wb.asBools)
  val fp_ldasbools = VecInit(fp_ld_wb.asBools)
  val rf_ld_addr = RegInit(VecInit(Seq.fill(32)(0.U(40.W))))
  val fp_ld_addr = RegInit(VecInit(Seq.fill(32)(0.U(40.W))))
...
//====================================wxx-runahead=============================//        
  ...
  //============================================runahead-mem=============================================//
    ...
    val mem_load_req_valid = Reg(Bool())
 ...
 //============================================runahead=============================================//
  ...
  mem_load_req_valid := io.dmem.req.valid
  //============================================runahead=============================================//
  ...
  when (mem_pc_valid) {
    ...
    wb_load_req_valid := mem_load_req_valid
  //==================================wxx-runahead================================//
 ...
 //=====================================runahead mode start==========================================================//
  ...
  wb_rh_longinst_stop_record := Mux(l2miss_falingedge||db_flag, false.B ,             //rh开始和结束时置低 遇到来源无效的分支就置高停止记录长指令的值
  ...
  //val fp_load_valid = wb_valid && wb_ctrl.wfd && wb_reg_load && wb_rh_longinst_valid && !io.fpu.sboard_set && !wb_rh_longinst_stop_record //立即写回的fp_load
  //val fp_load_waddr = Mux(wb_ctrl.wfd && wb_reg_load, wb_waddr, 0.U )
  // runahead期间记录长指令的结果
 when(if_l2miss){     //从rh开始记录
 //不写入scoreboard的指令
  when(wb_rh_longinst_valid && wb_wen && !wb_rh_longinst_stop_record)      //长指令来源有效，wb_wen = wb_valid && wb_ctrl.wxd 写回到rf，要用到rh_rf_waddr_step 
  {
...
//不写入scoreboard的浮点指令
 ...
 //写入scoreboard的指令
  when(ll_wen && rf_sb_wb(ll_waddr)===0.U && rh_rf_waddr_step(ll_waddr)=/=0.U && longinst_if_record(rh_rf_waddr_step(ll_waddr))===0.U){ //&& longinst_if_record(rh_rf_waddr_step(ll_waddr))===0.U){     // set_sboard后写回的指令   
...
//    }//.elsewhen (rf_wen) { rf.write(rf_waddr, rf_wdata) }
    ...
    val wb_reg_pc_r1 = RegNext (wb_reg_pc, init = 0.U)   
    val wb_reg_pc_r2 = RegNext (wb_reg_pc_r1, init = 0.U) 
    rcu.io.ipc := wb_reg_pc_r2
  ...
  //  //从进入rh开始，记录req.valid的load的目标寄存器和访存addr
  //   when(if_l2miss && wb_reg_load && wb_load_req_valid){}
  dontTouch(wb_load_req_valid)
 ...
 dontTouch(io.traceStall) 
  ...
  io.dmem.req.valid     := ex_reg_valid && ex_ctrl.mem && !ex_rh_store && !ex_longinst_kill //&& !db_flag        //wxx-runahead期间不写内存
